...
Page({
  ...
  async getVideoList(navId) {
    if (!navId) return;
    try {
      let videoListData = await request('/video/group', { id: navId, offset: 0 });
      wx.hideLoading();
      // console.log(videoListData);
      let index = 0;
      let videoDataList = await Promise.all(videoListData.datas.map(async item => {
        item.id = index++;
        let urlInfo = await request('/video/url', { id: item.data.vid });
        item.VideoUrl = urlInfo.urls[0].url;
        return item;
      }))
      this.setData({
  ...
  onShareAppMessage({ from }) {
    ...
    if (from === 'button') {
      return {
        ...
        imageUrl: '/static/images/xm.jpg'
      }
    } else {
      return {
        title: '来自menu的转发',
        page: '/pages/video/video',
        imageUrl: '/static/images/321.jpg'
      }
    }
  }
})